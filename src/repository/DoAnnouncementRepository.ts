...
export class DoAnnouncementRepository implements AnnouncementRepository {
  ...
  private mapToEntity(row: AnnouncementSchema): Announcement {
    ...
    // Set messages
    const messages = typeof row.message === 'object' ? row.message : JSON.parse(row.message as string);
    if (messages["en-US"]) {
    ...
    // Set readable roles
    const rolesArray = Array.isArray(row.roles) ? row.roles : JSON.parse(row.roles as string);
    const attendeeRoles = rolesArray
      .map((role: string) => role === "staff" ? AttendeeRole.STAFF : role === "audience" ? AttendeeRole.AUDIENCE : null)
      .filter((role: AttendeeRole | null): role is AttendeeRole => role !== null);
